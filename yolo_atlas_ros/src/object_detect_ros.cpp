...
ObjectDetect::ObjectDetect(const char *pkgPath,
                           const char *modelPath, uint32_t modelWidth,
...
Result ObjectDetect::InitResource() {
    ...
    const char *aclConfigPath = aclPath.c_str();
    cout << "aclConfigPath:" << endl << aclConfigPath << endl;
...
Result ObjectDetect::Postprocess(cv::Mat &frame,
                                 ...
                                 std::vector<BoundingBox> &detectionResults) {
...
//Result ObjectDetect::SendImage(vector<BoundingBox>& detectionResults,
//                                  cv::Mat& origImg) {
...
std::vector<BoundingBox> ObjectDetect::nonMaximumSuppression(const float nmsThresh, std::vector<BoundingBox> binfo) {
    ...
    auto computeIoU = [&overlap1D](BoundingBox &bbox1, BoundingBox &bbox2) -> float {
        float overlapX = overlap1D(bbox1.lt.x, bbox1.rb.x, bbox2.lt.x, bbox2.rb.x);
        float overlapY = overlap1D(bbox1.lt.y, bbox1.rb.y, bbox2.lt.y, bbox2.rb.y);
        float area1 = (bbox1.rb.x - bbox1.lt.x) * (bbox1.rb.y - bbox1.lt.y);
        float area2 = (bbox2.rb.x - bbox2.lt.x) * (bbox2.rb.y - bbox2.lt.y);
